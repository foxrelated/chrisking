	...
	<div class="table_right">
		<select name="val[user_type]" onchange="if ($(this).val() == {$iDealerUserGroup}){l}$('#dealer_extra_info').show();{r}else{l}$('#dealer_extra_info').hide();{r}">
			{foreach from=$aUserTypes key=sUserTypeKey item=sUserType}
				<option value="{$sUserTypeKey}" {if $sUserType =='Free Trial'}selected="selected"{/if}>{$sUserType}</option>
			{/foreach}
		</select>
	</div>
</div>

<div id="dealer_extra_info"{if $bSalesTeamInvite} style="display:none;"{/if}>
	<div class="table">
		<div class="table_left">
