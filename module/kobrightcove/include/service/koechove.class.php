...
class Kobrightcove_Service_Koechove extends Phpfox_Service {
	...
	{
		$this->token_read = '2m6A45-zsCYjkbFwSIXsrpWB2rJ9BPnHL586KzV4RcT-K9x4IHE4ag..';
		$this->token_write = '2m6A45-zsCZ2SeBLiwEL6aJwNlnNmyLQSZ6sfpvV107183PGmQVhRw..';
	...
	public function findUpdate($iPageNumber, $sCustomFields, $iPageSize = 100)
	{
		...
		$params['page_size'] = $iPageSize;
		$params['page_number'] = $iPageNumber;
		$params['custom_fields'] = $sCustomFields;
	...
	public function find($call, $params = NULL)
	{
		...
		switch ($call)
		{
			...
			case 'playlistsforplayerid':
				...
				break;
			default:
				return Phpfox_Error::set('ERROR_INVALID_METHOD');
				break;
		}
	...
	public function search($type = 'video', $terms = NULL, $params = NULL)
	{
		if (!isset($terms) || !is_array($terms)) {
			return Phpfox_Error::set('ERROR_SEARCH_TERMS_NOT_PROVIDED');
		}
	...
	public function createMedia($type = 'video', $file = NULL, $meta, $options = NULL)
	{
		if (strtolower($type) == 'video') {
			...
			return Phpfox_Error::set('ERROR_INVALID_TYPE');
		}
	...
	public function createImage($type = 'video', $file = NULL, $meta, $id = NULL, $ref_id = NULL, $resize = TRUE)
	{
		...
		if (isset($id)) {
			...
			return Phpfox_Error::set('ERROR_ID_NOT_PROVIDED');
		}
	...
	public function removeFromPlaylist($playlist_id, $video_ids)
	{
		...
		if (!isset($playlist)) {
			return Phpfox_Error::set('ERROR_DTO_DOES_NOT_EXIST');
		}
	...
	{
		return Phpfox_Error::set('ERROR_DEPRECATED');
	...
	private function getData($url)
	{
		...
		if (!isset($this->token_read)) {
			return Phpfox_Error::set('ERROR_READ_TOKEN_NOT_PROVIDED');
		}
		...
		if ($response && $response != 'NULL') {
			...
			if (isset($response_object->error)) {
				if ($this->timeout_retry && $response_object->error->code == 103 && $this->timeout_current < $this->timeout_attempts) {
					...
					return Phpfox_Error::set('ERROR_API_ERROR' . $response_object->error);
				}
			} else {
			...
			return Phpfox_Error::set('ERROR_API_ERROR');
		}
	}
	...
	private function putData($request, $return_json = TRUE)
	{
		if (!isset($this->token_write)) {
			return Phpfox_Error::set('ERROR_WRITE_TOKEN_NOT_PROVIDED');
		}
		...
		if ($return_json) {
			...
			if (!isset($response_object->result)) {
				return Phpfox_Error::set('ERROR_API_ERROR: ' . $response);
			}
		}
	...
	private function curlRequest($request, $get_request = FALSE)
	{
		...
		if ($curl_error !== NULL) {
			if ($get_request) {
				return Phpfox_Error::set('ERROR_READ_API_TRANSACTION_FAILED' . $curl_error);
			} else {
				return Phpfox_Error::set('ERROR_WRITE_API_TRANSACTION_FAILED' . $curl_error);
			}
		}
