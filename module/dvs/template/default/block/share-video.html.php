            ...
            <tr>
                <td>
					...
					<td>
						...
						<input class="dvs_share_text_box" type="text" id="embed_code_{$iKey}" value='<div style="position:relative;width:300px;overflow:hidden;text-align:center;"><a href="{url link='share.'$aVideo.share_hash_code'3'}"><span style="text-decoration:none;font-weight:bold;">{$aVideo.name}</span></a><div style="height:100%;left:0;top:0;width:300px;">	<a href="{url link='share.'$aVideo.share_hash_code'3'}">{img server_id=$aVideo.image_server_id path="brightcove.url_image" file=$aVideo.image_path suffix="_email_300" max_width=300 max_height=300 title=$aVideo.name}</a></div></div>
						' />
					</td>
					<td>
						{if !$bIsIPhone}
