...
#dvs_vin_popup_wrapper {l}
    opacity: 0;
    display: none;
    height: {if $sBrowser == 'mobile'}{$iPopupHeight}px{else}550px{/if};
    left: 0;
    position: fixed;
    _position:absolute;
    top: 0;
...
#dvs_vin_popup {l}
    display: block;
    width: 90%;
    max-width:{if $sBrowser != 'mobile'}880px{elseif $sBrowser == 'ipad'}860px{else}815px{/if};
    position: relative;
    margin: 0 auto;
    height: 100%;
    max-height:{if $sBrowser == 'mobile'}{$iPopupHeight}px{else}435px{/if};
...
#dvs_vin_popup_content {l}
    display: block;
    width: 100%;
    max-width:{if $sBrowser != 'mobile'}895px{elseif $sBrowser == 'ipad'}860px{else}815px{/if};
    height: 100%;
    max-height:{if $sBrowser == 'mobile'}{$iPopupHeight}px{else}432px{/if};
    border-radius: 5px;
    overflow: hidden;
    position: relative;
    z-index:10001;
