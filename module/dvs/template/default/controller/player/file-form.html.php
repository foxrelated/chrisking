...
<script type="text/javascript">
		$(document).ready(function() {left_curly}
			...
			{left_curly}
				if (!empty(this.value))
				{left_curly}
					$('#js_file_form').ajaxCall('dvs.fileProcess');
				{right_curly}
			{right_curly});
		{right_curly});
</script>
	...
	<div id="js_file_content">
		<div id="js_file_done" style="display:none;">
			<div class="valid_message">
				{phrase var='dvs.file_successfully_uploaded'}
			</div>
		</div>

		<div id="js_file_process" style="display:none;">
			<div class="message">
				{img theme='ajax/small.gif' alt='' class='v_middle'} <span id="js_upload_command">{phrase var='dvs.uploading'}</span>: <span id="js_upload_file_name"></span>
			</div>
		</div>	
		<form method="post" action="{url link='dvs.player.file-Frame'}" id="js_file_form" enctype="multipart/form-data" target="js_upload_frame">
